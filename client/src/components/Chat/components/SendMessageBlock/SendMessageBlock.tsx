...
const SendMessageBlock = () => {
  ...
  const handleStartTyping = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    socket.emit(
      "message:startTyping",
      `${useUserStore.getState().user?.username} is typing`
    );
  };
  ...
  useEffect(() => {
    socket.on("message:responseStartTyping", (data: string) =>
      changeTypingStatus(data)
    );
  }, [socket]);

  useEffect(() => {
    socket.on("message:responseEndTyping", (data: string) =>
      changeTypingStatus(data)
    );
  }, [socket]);
    ...
    <>
      <form onSubmit={handleMessage} className={styles.message_box}>
        ...
        <div className={styles.message_box__controls}>
          <button
            type="button"
            className={styles.message_box__emoji_toggler}
            onClick={toggleEmojiPicker}
          >
            {currentEmoji ? currentEmoji : <EmojiIcon />}
          </button>
          <button className={styles.message_box__send_message} type="submit">
            <SendMessageIcon />
          </button>
        </div>
      </form>
